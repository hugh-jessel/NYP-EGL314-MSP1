...
def reactionTest():
    ...
    buttonPressed = False
    game_fail = False
    last_button_press_time = 0  # Timestamp of the last button press
    button_press_delay = 4  # Delay in seconds to reset the buttonPressed flag
    last_note_on_time = time.time()  # Timestamp of the last note_on message
    note_timeout_threshold = 5  # Time in seconds before game-over due to inactivity
    game_active = False  # Flag to indicate when the game is in the active state
    successful_deflects = 0  # Counter for successful deflects
    win_threshold = 4  # Number of successful deflects required to win the game
        ...
        try:
            while True:
                ...
                if gameCount >= 5:
                    ...
                    print(f"gameCount ({gameCount}) is less than 5, game not active yet")

                for msg in inport.iter_pending():
                    if game_active:  # Process input only if the game is active
                        if msg.type == "note_on":
